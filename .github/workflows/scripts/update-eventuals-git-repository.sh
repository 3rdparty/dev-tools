...
if [[ ${#commit} == 0 ]]; then
    echo "Incorrect commit value."
    exit 1
fi

if [[ ${#shallow_since} == 0 ]]; then
    echo "Incorrect shallow_since value."
    exit 1
fi

# This flag indicates whether or not we process correct
# 'git_repository' rule for eventuals in current $file.
git_repository_needed=False

# Flags to help understand whether or not all updates
...
do
  ...
  if [[ "${line}" =~ .*"commit = ".* && $git_repository_needed = True ]];
  then
  ...
  if [[ "${line}" =~ .*"shallow_since = ".* && $git_repository_needed = True ]];
  then
  ...
  if [[ $is_commit_replaced = True && $is_shallow_replaced = True ]];
  then
    break
  fi
done < "$file"
